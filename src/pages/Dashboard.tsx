...
function StatusLine({ log }: { log: FootprintLog }) {
  ...
  if (diff < -5) {
    return (
      <p className="flex items-center gap-1.5 text-sm font-medium text-emerald-400 mt-1">
  ...
  if (diff > 5) {
    return (
      <p className="flex items-center gap-1.5 text-sm font-medium text-amber-400 mt-1">
        <TrendingUp size={14} />
...
function HeroBand({ profile, todayLog }: { profile: UserProfile | null; todayLog: FootprintLog | null }) {
  return (
    <div className="mb-6 pb-5 border-b border-white/5">
      <p className="text-xs font-medium text-emerald-500/70 tracking-widest uppercase">{formatDate()}</p>
      ...
      {todayLog ? (
        ...
        <p className="text-sm text-zinc-500 mt-1">Log your footprint to see how you compare.</p>
      )}
    </div>
  )
}
...
function AverageRow() {
  return (
    <div className="flex gap-3 mt-5">
      {[
        { label: 'India avg', value: INDIA_AVERAGE_KG_CO2_PER_MONTH, color: 'text-amber-400', bg: 'bg-amber-500/10 border border-amber-500/15' },
        { label: 'Global avg', value: GLOBAL_AVERAGE_KG_CO2_PER_MONTH, color: 'text-red-400', bg: 'bg-red-500/10 border border-red-500/15' },
      ].map(({ label, value, color, bg }) => (
        <div key={label} className={`flex-1 ${bg} rounded-2xl px-4 py-3 text-center`}>
          <p className="font-data text-lg font-semibold text-white">{value}</p>
          <p className={`text-xs font-medium ${color}`}>{label}</p>
          <p className="text-[10px] text-zinc-600">kg CO₂ / mo</p>
        </div>
      ))}
    </div>
  )
    ...
    <section aria-label="Carbon footprint dashboard" className="animate-slide-up">
      ...
      {!todayLog ? (
        <div className="card p-8 text-center">
          <div className="w-14 h-14 bg-emerald-500/10 border border-emerald-500/15 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <TrendingDown size={24} className="text-emerald-400" />
          </div>
          <p className="font-display font-semibold text-white">No log for today yet</p>
          <p className="text-sm text-zinc-500 mt-1">
            Tap <strong className="text-zinc-300">Log</strong> in the navigation to record your footprint.
          </p>
        </div>
      ) : (
        <>
